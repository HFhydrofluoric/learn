...
citySelector.cityInit = function (input) {
    ...
    jQuery("#" + input).click(function () {
        ...
        jQuery("#js_cityBox").remove();
        jQuery("body").append(citySelector.template);

        var _top = jQuery(this).offset().top + 40,
            _left = jQuery(this).offset().left,
            _width = jQuery(window).width();
        if (_width - _left < 450) {
            jQuery("#js_cityBox").css({ "top": _top + "px", "right": "0px" }).addClass("rightSelector");
        }
        else {
            jQuery("#js_cityBox").css({ "top": _top + "px", "left": _left + "px" });
        }

        var label = "false";
        jQuery("#js_provList").on("click", ".provinceName", function () {
            function createUl(_this){
                ...
                cityHtmls += "</ul>";
                jQuery("#js_provCitys").remove();
                _this.parent("li").append(cityHtmls);
            }

            if (label == "false") {
                label = "true";
                jQuery(this).attr("now-item", "true");
                createUl(jQuery(this));
            }
            else {
                if (jQuery(this).attr("now-item") == "" || jQuery(this).attr("now-item") == "false" || jQuery(this).attr("now-item") == undefined) {
                    jQuery(this).parents("#js_provList").find("span").attr("now-item", "false");
                    jQuery(this).attr("now-item", "true");
                    jQuery("#js_provList span").css({ "background": "", "border-color": "", "border-bottom-color": "", "position": "", "top": "", "left": "", "z-index": "" });
                    jQuery("#js_provCitys").remove();
                    createUl(jQuery(this));
                }
                else {
                    label = "false";
                    jQuery(this).css({ "background": "", "border-color": "", "border-bottom-color": "", "position": "", "top": "", "left": "", "z-index": "" });
                    jQuery("#js_provCitys").remove();
                }
            }
        ...
        var _input = input;
        jQuery("#js_cityBox").on("click", ".js_cityName", function (e) {
            e.stopPropagation();
            jQuery("#" + _input).val(jQuery(this).html());
            jQuery("#js_cityBox").remove();
        });
    });
}
